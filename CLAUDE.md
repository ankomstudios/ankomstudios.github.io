...
Partially resolved as of 2026-09-12: `.github/workflows/static.yml` confirms **GitHub Actions → GitHub Pages** is an active deploy path (checkout → upload-pages-artifact → deploy-pages, triggered on push to `main`). However, a `wrangler.jsonc` (Cloudflare Workers static-assets config, `directory: "."`) was also added recently — still need to confirm with the user whether that's a second live deploy target for the domain, a migration-in-progress away from GitHub Pages, or leftover/unused. Matters because Workers static assets don't share GitHub Pages' Git LFS limitation, though moot either way since media bypasses git entirely now.
...
4. **Confirm the deploy pipeline** (see Open item above) — specifically, which of GitHub Pages vs. the new Wrangler/Workers static-assets config is the one actually serving ankomstudios.com.

5. **Yeezy Night Funkin universal build — done.** Created `/ynf/index.html` (2026-09-12), a single-card download page mirroring `/buy/`'s pattern, wired to `data-download-file="downloads/ynf-universal-build-v1.0.5.zip"`. Uploaded `universal-build-v1.0.5.zip` (239,048,638 bytes) to that exact R2 path via boto3/S3-API (see auth gotcha above) and verified `https://raw.ankomstudios.com/downloads/ynf-universal-build-v1.0.5.zip` returns `200 OK` with matching `Content-Length`. Download button on `/ynf/` is fully working end-to-end. Page is **not yet linked** from the nav/home page — still needs that if it should be publicly discoverable.
